...
import { listRuntimeNotifications } from "@/lib/state/runtime-store";

export async function handleListNotificationsRequest(
  request: Request,
  env: Env,
): Promise<Response> {
  ...
  return jsonResponse(
    buildSuccessPayload(
      {
        notifications: listRuntimeNotifications(auth.user.id),
      },
      requestId,
    ),
  );
